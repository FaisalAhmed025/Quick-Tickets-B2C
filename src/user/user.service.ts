...
export class UserService {
  ...
  async Register(
    ...
    const newuser= await this.userpository.create({...authDto, password: hashedPassword});
 ...
 // generate token 
 async generateToken(authDto: CreateUserDto): Promise<string> {
    const payload = { email: authDto.email, userID: authDto.userID};
    console.log(payload);
    const token =  await this.jwtService.signAsync(payload)
    authDto.access_token = token;
    await this.userpository.save(authDto);
    return token;
  }
  ...
  async sendRegisterSuccecess(authdto:CreateUserDto) {
    ...
    const mailOptions = {
      ...
      subject: 'Welcome To Quick ticktes',
